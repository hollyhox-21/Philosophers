...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/17 17:39:00 by rjoella           #+#    #+#             */
/*   Updated: 2021/05/17 18:00:37 by rjoella          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_sem(t_table *t)
{
	...
	sem_unlink("fork");
	sem_unlink("locker");
	t->locker = sem_open("locker", O_CREAT, 0644, 1);
	if (t->locker == SEM_FAILED)
		return (0);
	...
	if (t->death == SEM_FAILED)
		return (0);
	t->waiter = sem_open("waiter", O_CREAT, 0644, 1);
	if (t->waiter == SEM_FAILED)
		return (0);
	t->forks = sem_open("fork", O_CREAT, 0644, t->count_philos);
